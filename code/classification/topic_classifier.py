...
This module implements a 15-topic classification system for analyzing X (Twitter) Community Notes.
The classifier covers conflicts, geopolitics, and general categories including:
- Conflicts: Ukraine, Gaza, Syria, Iran, China-Taiwan, China Influence, Other Conflicts
- General Topics: Scams, Health/Medical, Climate/Environment, Politics, Technology, 
  Economics, Entertainment, Immigration

Author: Omer Bar Lev, Yair Ben Eliyahu
Date: 2025
"""
...
def detect_language(text: str) -> bool:
    """
    Detect if text is in English using available language detection libraries.
    
    Args:
        text: Input text to check
        
    Returns:
        True if text appears to be in English, False otherwise
    """
    ...
    try:
        if LANGDETECT_AVAILABLE:
            ...
            return english_count >= MIN_ENGLISH_WORDS
    except Exception:
...
class CustomTopicClassifier:
    ...
    This classifier implements a 15-topic classification system using TF-IDF vectorization
    and Logistic Regression. It covers conflicts, geopolitics, and general categories.
    
    Attributes:
        data_path: Path to the data directory
        output_dir: Path to output directory for results
        trained_pipeline: Trained sklearn Pipeline (TF-IDF + Logistic Regression)
        topic_labels: Dictionary mapping topic IDs to topic names
        seed_terms: Dictionary of seed terms for each topic category
    """
    
    def __init__(self, data_path: str | Path = "data", output_dir: str | Path = "custom_topic_results"):
        """
        Initialize the CustomTopicClassifier.
        
        Args:
            data_path: Path to the data directory containing Community Notes
            output_dir: Path to directory for saving classification results
        """
        self.data_path = Path(data_path)
        self.trained_pipeline: Optional[Pipeline] = None
        self.topic_labels: Optional[Dict[int, str]] = None
        self.seed_terms: Optional[Dict[str, set]] = None
    ...
    def setup_custom_topics(self) -> None:
        """
        Initialize custom topic categories and seed terms.
        
        Sets up 15 topic categories with their corresponding seed terms for
    ...
    def create_custom_seed_terms(self) -> Dict[str, set]:
        """
        Create custom seed terms for all 15 topic categories.
        
        Returns:
            Dictionary mapping topic names to sets of seed terms
        """
    ...
    def assign_seed_labels(
        ...
        Args:
            notes_df: DataFrame containing Community Notes with 'summary' column
            progress_tracker: Optional progress tracker for long-running operations
    ...
    def train_topic_classifier(
        ...
        """
        Train TF-IDF + Logistic Regression classifier on labeled data.
        
        Args:
            notes_df: DataFrame containing Community Notes with 'summary' column
            seed_labels: Array of seed-assigned topic labels
    ...
    def classify_all_notes(
        ...
        """
        Apply trained classifier to all notes in the dataset.
    ...
    def analyze_classification_results(self, classified_notes: pd.DataFrame) -> Dict[str, Any]:
        """
        Analyze and display classification results.
        
        Args:
            classified_notes: DataFrame with classification results including 
                           'topicName' and 'confidence' columns
    ...
    def save_all_outputs(
        self, 
        classified_notes: pd.DataFrame, 
        trained_pipeline: Pipeline, 
        metadata: Dict[str, Any], 
        analysis_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Save all classification outputs to files.
        
        Args:
            classified_notes: DataFrame with classification results
            trained_pipeline: Trained sklearn Pipeline to save
            metadata: Processing metadata dictionary
            analysis_results: Analysis results dictionary
            
        Returns:
            Dictionary containing file paths and summary data
        """
        print("\n" + "="*70)
    ...
    def load_community_notes_data(
        ...
        Args:
            english_only: If True, filter to English-only notes
            force_refilter: If True, force re-filtering even if cached file exists
            
        Returns:
            DataFrame containing Community Notes
            
        Raises:
            FileNotFoundError: If notes file is not found
    ...
    def run_complete_pipeline(
        ...
        1. Load Community Notes data
        2. Assign seed labels based on keyword matching
        3. Train TF-IDF + Logistic Regression classifier
        ...
        Returns:
            Dictionary containing summary data and file paths
    ...
    def show_seed_term_examples(self) -> None:
        """Display examples of seed terms for each topic category."""
        print("\n" + "="*70)
    ...
    def load_data_and_preview_topics(self, max_preview: int = 1000) -> Dict[str, Dict[str, Any]]:
        """
        Load data and preview how many notes would match each topic.
        
        Args:
            max_preview: Maximum number of notes to preview
            
        Returns:
            Dictionary mapping topic names to match statistics
        """
        print("\n" + "="*70)
...
def main() -> None:
    ...
    Shows seed term examples and previews topic matching on sample data.
    """
    print("="*70)
...
def run_test() -> Dict[str, Any]:
    """
    Run a test classification with 10K notes with English filtering.
    
    Returns:
        Dictionary containing classification summary data
    """
    print("="*70)
...
def run_test_50k() -> Dict[str, Any]:
    """
    Run a test classification with 50K notes with English filtering.
