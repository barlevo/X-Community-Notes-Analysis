...
This module analyzes the classification results from the full dataset,
providing comprehensive statistics and visualizations.
...
def load_and_analyze_results(
    results_dir: str | Path = "custom_topic_results",
    ...
    Args:
        ...
        results_file: Specific results CSV file (if None, uses most recent)
        summary_file: Specific summary JSON file (if None, uses most recent)
        
    Returns:
        Tuple of (topic_analysis_list, categories_dict)
    """
    results_dir = Path(results_dir)
    
    # Find most recent files if not specified
    if results_file is None:
        csv_files = list(results_dir.glob("classified_notes_*.csv"))
        if not csv_files:
            raise FileNotFoundError(f"No classification results found in {results_dir}")
    ...
    if summary_file is None:
        ...
        summary_file = max(json_files, key=lambda x: x.stat().st_mtime)
    else:
...
def create_visualizations(
    ...
    output_file: str | Path = "community_notes_analysis.png"
) -> plt.Figure:
    """
    Create visualizations of the results.
    
    Args:
        topic_analysis: List of topic analysis dictionaries
